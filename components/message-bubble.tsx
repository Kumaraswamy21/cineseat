...
export function MessageBubble({
  ...
  // After cancelBooking finishes, the tool result is the source of truth.
  // The model's follow-up stream often repeats or garbles itself on multi-step
  // tool flows, so we hide the extra assistant text in that case.
  const cancelFinished = message.toolInvocations?.some(
    (invocation) =>
      invocation.toolName === "cancelBooking" && invocation.state === "result",
  );

  return (
    <div className={`bubble bubble--${message.role}`}>
      {message.content && !cancelFinished && (
